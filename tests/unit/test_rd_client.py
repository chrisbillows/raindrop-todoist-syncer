...
@pytest.fixture
def rd_client_simple_init():
    """
    Use simplified test object for all tests apart from refresh token functionality.
    """
    return RaindropClientSimpleInit()


class TestInit:
    def test_api_url(self, rd_client_simple_init):
        assert rd_client_simple_init.BASE_URL == "https://api.raindrop.io/rest/v1"

    def test_oauth_token(self, rd_client_simple_init):
        assert rd_client_simple_init.raindrop_access_token is not None

    def test_headers(self, rd_client_simple_init):
        assert rd_client_simple_init.headers is not None

    def test_header_structure_key_is_authorization(self, rd_client_simple_init):
        headers = rd_client_simple_init.headers
        assert list(headers.keys())[0] == "Authorization"

    def test_header_structure__value_starts_bearer(self, rd_client_simple_init):
        headers = rd_client_simple_init.headers
        assert str(list(headers.values())[0]).startswith("Bearer")


class TestInitRefreshingToken:
    """
    Tests for refresh token functionality. Use real object, not test object.
    """
...
class TestStaleToken:
    def test_200_response(self, rd_client_simple_init):
        with patch(
            ...
            result = rd_client_simple_init.stale_token()
            assert not result

    def test_401_response(self, rd_client_simple_init):
        with patch(
            "raindrop_todoist_syncer.rd_client.RaindropClient._core_api_call",
    ...
    def test_404_response(self, rd_client_simple_init):
        with patch(
            "raindrop_todoist_syncer.rd_client.RaindropClient._core_api_call",
            side_effect=requests.exceptions.HTTPError(response=Mock(status_code=404)),
        ):
            with pytest.raises(HTTPError):
                rd_client_simple_init.stale_token()


class TestGetAllRaindrops:
    def test_get_all_raindrops_len(self, mock_requests_get, rd_client_simple_init):
        result = rd_client_simple_init.get_all_raindrops()
        assert len(result) == 26

    def test_get_all_raindrops_type(self, mock_requests_get, rd_client_simple_init):
        result = rd_client_simple_init.get_all_raindrops()
        assert isinstance(result, list)

    def test_get_all_raindrops_ids(self, mock_requests_get, rd_client_simple_init):
        expected_ids = [
            628161680,
            628161679,
            628161678,
...
class TestCoreApiCall:
    ...
    def test__core_api_call_cached_responses_not_none(
        self, mock_requests_get, rd_client_simple_init, page
    ):
        response = rd_client_simple_init._core_api_call(page)
    ...
    def test__core_api_call_cached_responses_status_200(
        ...
        response_status = rd_client_simple_init._core_api_call(page).status_code
        assert response_status == 200

    def test_core_api_call_cached_responses_missing_pg_exception(
        self, mock_requests_get, rd_client_simple_init
    ):
        with pytest.raises(HTTPError):
            rd_client_simple_init._core_api_call(10).status_code
    ...
    def test_core_call_cached_responses(
        self, mock_requests_get, rd_client_simple_init, page, key_path, expected_value
    ):
        response = rd_client_simple_init._core_api_call(page)
...
class TestMakeApiCall:
    ...
    def test__make_api_call_cached_responses_not_none(
        ...
        response = rd_client_simple_init._make_api_call(page)
        assert response is not None
    ...
    def test__make_api_call_cached_responses_status_200(
        ...
        response_status = rd_client_simple_init._make_api_call(page).status_code
        assert response_status == 200
    ...
    def test_makes_api_call_retries_for_various_status_codes(
        ...
        if exception:
            with pytest.raises(exception, match=match_str):
                rd_client_simple_init._make_api_call(0)
            assert mock_get.call_count == call_count
        else:
            assert rd_client_simple_init._make_api_call(0) == mock_response
            assert mock_get.call_count == call_count
...
class TestExtractBenchmarkCount:
    ...
    def test_extract_benchmark_count(
        self, request, rd_client_simple_init, fixture_name
    ):
        data = request.getfixturevalue(fixture_name)
        benchmark_count = rd_client_simple_init._extract_benchmark_count(data)
        assert benchmark_count == 26

    def test_extract_benchmark_count_700(self, rd_client_simple_init):
        data = {"count": 700}
        benchmark_count = rd_client_simple_init._extract_benchmark_count(data)
        assert benchmark_count == 700

    def test_extract_benchmark_set_to_none(self, rd_client_simple_init):
        data = {"count": None}
        with pytest.raises(
            ValueError,
            match="The 'count' key was found in the response data, but its value was None.",
        ):
            rd_client_simple_init._extract_benchmark_count(data)

    def test_extract_benchmark_negative_count(self, rd_client_simple_init):
        data = {"count": -300}
        with pytest.raises(
            ValueError,
    ...
    def test_extract_benchmark_count_not_present(self, rd_client_simple_init):
        data = {}
        with pytest.raises(
...
class TestCalculateMaxPages:
    def test_calculate_max_pages_response_one(
        self, rd_client_simple_init, response_one_data
    ):
        benchmark_rd_count = response_one_data["count"]
        assert rd_client_simple_init._calculate_max_pages(benchmark_rd_count) == 2

    def test_calculate_max_pages_random_allowable_num(self, rd_client_simple_init):
        max_rds = (
            rd_client_simple_init.MAX_ALLOWED_PAGES
            * rd_client_simple_init.RAINDROPS_PER_PAGE
        )
        benchmark_rd_count = random.randint(1, max_rds)
        whole_pages = benchmark_rd_count // rd_client_simple_init.RAINDROPS_PER_PAGE
        partial_pages = benchmark_rd_count % rd_client_simple_init.RAINDROPS_PER_PAGE
        if partial_pages:
            whole_pages += 1
        assert (
            rd_client_simple_init._calculate_max_pages(benchmark_rd_count)
            == whole_pages
        )

    def test_calculate_max_pages_error(self, rd_client_simple_init):
        benchmark_rd_count = (
            rd_client_simple_init.MAX_ALLOWED_PAGES
            * rd_client_simple_init.RAINDROPS_PER_PAGE
            + 1
...
class TestDataValidator:
    def test_data_validator_result_true(self, rd_client_simple_init, response_one_data):
        assert rd_client_simple_init._data_validator(response_one_data, 26) is None

    def test_data_validator_result_false(self, rd_client_simple_init):
        data = {"result": False}
        with pytest.raises(ValueError, match="API Result False"):
            rd_client_simple_init._data_validator(data, 26)

    def test_data_validator_benchmark_count_change(
        self, rd_client_simple_init, response_one_data
    ):
        ...
        with pytest.raises(
            ...
            rd_client_simple_init._data_validator(response_one_data, benchmark_rd_count)


class TestsIndividualRdValidator:
    @pytest.mark.parametrize("fixture_name", ["response_one_data", "response_two_data"])
    def test__individual_rd_validator(
        self, request, rd_client_simple_init, fixture_name
    ):
        data = request.getfixturevalue(fixture_name)
        rds = data["items"]
        assert rd_client_simple_init._individual_rd_validator(rds) is None

    def test__individual_rd_validator_manual_ids_pass(self, rd_client_simple_init):
    ...
    def test__individual_rd_validator_manual_ids_fails(
        self, rd_client_simple_init, rds, match_str
    ):
        rds = rds
        with pytest.raises(ValueError, match=match_str):
            rd_client_simple_init._individual_rd_validator(rds)
...
class TestCumulativeRdsValidator:
    ...
    def test_cumulative_rds_validator(
        ...
        if exception is not None:
            with pytest.raises(exception, match=match_str):
                rd_client_simple_init._cumulative_rds_validator(
                    cumulative_rds, current_rds, bmc
                )
        else:
            assert (
                rd_client_simple_init._cumulative_rds_validator(
    ...
    def test_cumulative_rds_validator_manual_pass(self, rd_client_simple_init):
        cumulative_rds = list(range(1772))
        current_rds = list(range(22))
        bm = 1772
        assert (
            rd_client_simple_init._cumulative_rds_validator(
                cumulative_rds, current_rds, bm
            )
            is None
        )

    def test_cumulative_rds_validator_manual_fail(self, rd_client_simple_init):
        cumulative_rds = list(range(1772))
        current_rds = list(range(22))
        bm = 1771
        with pytest.raises(
    ...
    def test_cumulative_rds_last_pg_wrong_length(self, rd_client_simple_init):
        cumulative_rds = list(range(53))
        current_rds = list(range(25))
        bm = 53
        expected_len_last_page = bm % rd_client_simple_init.RAINDROPS_PER_PAGE
        with pytest.raises(
            ValueError,
