...
class TestOauthInit:
    ...
    def test_base_url_correct(self, raindrop_oauth):
        assert (
            raindrop_oauth.AUTH_CODE_BASE_URL == "https://raindrop.io/oauth/authorize"
        )
...
class TestOpenAuthCodeUrl:
    ...
    def test_open_authorization_code_url_creates_url_correctly(
        self, mock_webbrowser_open, raindrop_oauth
    ):
...
class TestParseAuthorizationCodeUrl:
    def test_parse_auth_code_url_valid(self, raindrop_oauth):
        authorization_code = raindrop_oauth._parse_authorization_code_url(
            "http://localhost/?code=aa4c0bc8-0e19-4615-a032-bd3379829ca7"
        )
...
class TestExtractOauthToken:
    def test_extract_oauth_token(self, raindrop_oauth, response_object_200):
        assert (
            raindrop_oauth._extract_oauth_token(response_object_200)
...
class TestCreateUpdatedEnvBody:
    ...
    def test_only_one_raindrop_oauth_token_present(
        self, raindrop_oauth, request, mock_env
    ):
    ...
    def test_num_of_lines_correct(
        self, raindrop_oauth, request, mock_env, expected_lines
    ):
...
class TestNewEnvValidator:
    def test_valid_simple_list(
        self, raindrop_oauth, request, placeholder_one_liner_env
    ):
        """
    ...
    def test_invalid_duplicate_token_middle_last(
        self, raindrop_oauth, full_env_oauth_middle
    ):
    ...
    def test_invalid_duplicate_token_both_at_end(
        self, raindrop_oauth, full_env_oauth_last
    ):
        mocked_open = mock_open(read_data=full_env_oauth_last)
